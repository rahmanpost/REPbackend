...
// Build a minimal spec in-memory (no file needed)
const serverUrl = process.env.API_ORIGIN || `http://localhost:${process.env.PORT || 5000}`;

const spec = {
  openapi: '3.0.3',
  info: { title: 'Rahman Express Post API', version: '1.0.0' },
  ...
  components: {
    ...
    schemas: {
      ...
      CreateShipmentRequest: {
        type: 'object', required: ['pickupAddress','deliveryAddress','boxType'],
        properties: {
          sender: { type: 'string' },
          pickupAddress: { type: 'object' },
          deliveryAddress: { type: 'object' },
          boxType: { type: 'object', properties: { kind: { type: 'string', enum: ['PRESET','CUSTOM'] } }, required: ['kind'] },
      ...
      post: {
        ...
        requestBody: {
          ...
          content: { 'application/json': { schema: {
            type: 'object',
            properties: { to: { type: 'string', format: 'email' }, subject: { type: 'string' }, html: { type: 'string' }, text: { type: 'string' } },
            required: ['to']
          } } }
        },
